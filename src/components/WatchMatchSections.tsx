...
import { buildMatchContext } from '../lib/watchProse/cat1b'
import { runCat2, type Cat2Output } from '../lib/watchProse/cat2'

interface WatchMatchSectionsProps {
  detail: ODMatchDetail
}

/**
 * Renders the four match-level cards (Draft / Lane / Mid game / Teamfights)
 * below the per-player grid on /watch/{match_id}. Each card maps to one
 * Cat 2 sub-section. Empty sub-sections render NOTHING — no apologetic
 ...
 */
export function WatchMatchSections({ detail }: WatchMatchSectionsProps) {
  ...
  return (
    <div className="dwr-watch-match-sections">
...
function Section({
  ...
  return (
    <article className="card dwr-watch-match-section">
      <h2 className="dwr-watch-match-section-label">{label}</h2>
      <div className="dwr-watch-match-section-body">
        {fires.map((f) => (
          <p key={f.templateId} className="dwr-watch-match-section-prose">
            {f.text}
          </p>
        ))}
      </div>
